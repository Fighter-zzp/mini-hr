      ...
      <div style="display: flex;justify-content: space-between">
        ...
        <div>
          <el-upload
            ...
            :headers="headers"
            action="http://localhost:8081/api/pims/employee/basic/import">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      exportData() {
        window.open('http://localhost:8081/api/pims/employee/basic/export?access_token='
          +VueCookies.get('token'), '_parent');
      },
      emptyEmp() {
        this.emp = {
          name: "",
      ...
      deleteEmp(data) {
        this.$confirm('此操作将永久删除【' + data.name + '】, 是否继续?', '提示', {
          ...
          this.deleteRequest("/pims/employee/basic/" + data.id).then(resp => {
            if (resp) {
              this.initEmps();
            }
          })
      ...
      doAddEmp() {
        if (this.emp.id) {
          this.$refs['empForm'].validate(valid => {
            if (valid) {
              this.putRequest("/pims/employee/basic/", this.emp).then(resp => {
                if (resp) {
          ...
          this.$refs['empForm'].validate(valid => {
            if (valid) {
              this.postRequest("/pims/employee/basic/", this.emp).then(resp => {
                if (resp) {
                  this.dialogVisible = false;
                  this.initEmps();
                }
      ...
      initPositions() {
        this.getRequest('/pims/employee/basic/positions').then(resp => {
          if (resp) {
            this.positions = resp;
          }
        })
      ...
      getMaxWordID() {
        this.getRequest("/pims/employee/basic/maxWorkID").then(resp => {
          if (resp) {
      ...
      initData() {
        if (!window.sessionStorage.getItem("nations")) {
          this.getRequest('/pims/employee/basic/nations').then(resp => {
            if (resp) {
              this.nations = resp;
        ...
        if (!window.sessionStorage.getItem("joblevels")) {
          this.getRequest('/pims/employee/basic/joblevels').then(resp => {
            if (resp) {
              this.joblevels = resp;
        ...
        if (!window.sessionStorage.getItem("politicsstatus")) {
          this.getRequest('/pims/employee/basic/politicsstatus').then(resp => {
            if (resp) {
        ...
        if (!window.sessionStorage.getItem("deps")) {
          this.getRequest('/pims/employee/basic/deps').then(resp => {
            if (resp) {
              this.allDeps = resp;
      ...
      initEmps(type) {
        this.loading = true;
        let url = '/pims/employee/basic/?page=' + this.page + '&size=' + this.size;
