        ...
        methods: {
            ...
            deleteSalary(data) {
                this.$confirm('此操作将删除【' + data.name + '】账套，是否继续？', '提示', {
                    ...
                    this.deleteRequest("/pims/salary/sob/" + data.id).then(resp => {
                        if (resp) {
                            this.initSalaries();
                        }
                    })
            ...
            nextStep() {
                if (this.activeItemIndex === 10) {
                    if (this.salary.id) {
                        this.putRequest("/pims/salary/sob/", this.salary).then(resp=>{
                            if (resp) {
                                this.initSalaries();
                                this.dialogVisible = false;
                            }
                        ...
                        this.postRequest("/pims/salary/sob/", this.salary).then(resp => {
                            if (resp) {
                                this.initSalaries();
                                this.dialogVisible = false;
                            }
            ...
            initSalaries() {
                this.getRequest("/pims/salary/sob/").then(resp => {
                    if (resp) {
                        this.salaries = resp;
                    }
                })
