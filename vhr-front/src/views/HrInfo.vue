      ...
      <div>
        <div style="display: flex;justify-content: center">
          <el-upload
            ...
            :data="hr"
            :headers="headers"
            action="http://localhost:8081/api/pims/hr/userface">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      updateHrInfo() {
        this.putRequest("/pims/hr/info", this.hr2).then(resp => {
          if (resp) {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.putRequest("/pims/hr/pass", this.ruleForm).then(resp => {
              if (resp) {
      ...
      initHr() {
        this.getRequest('/pims/hr/info').then(resp => {
          if (resp) {
            this.hr = resp;
